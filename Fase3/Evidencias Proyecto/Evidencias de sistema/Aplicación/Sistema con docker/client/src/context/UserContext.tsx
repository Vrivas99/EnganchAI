...
export const UserProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
    ...
    const fetchDataUser = async () => {
        try {
            const response = await fetch(`${process.env.NEXT_PUBLIC_EXPRESS_SERVER_URL}/db/getToken`, {
                method: 'GET',
                credentials: 'include',
                headers: {
    ...
    const logout = async () => {
        try {
            await fetch(`${process.env.NEXT_PUBLIC_EXPRESS_SERVER_URL}/db/deleteToken`, {
                method: 'GET',
                credentials: 'include',
                headers: {
                    'Content-Type': 'application/json',
